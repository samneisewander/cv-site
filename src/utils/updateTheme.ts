...
export const updateTheme = (
  ...
  const DARK_SELECTOR = Array.isArray(darkModeConfig) ? darkModeConfig[0] === "class" ? darkModeConfig[1] || ".dark" : "@media (prefers-color-scheme: dark)" : darkModeConfig === "class" ? ".dark" : "@media (prefers-color-scheme: dark)";
    ...
    Object.keys(colors).forEach((lightColorName) => {
      ...
      if (match) {
        ...
        if (lightColor && darkColor) {
          ...
          `
          if (DARK_SELECTOR === "@media (prefers-color-scheme: dark)"){
            newCSS += `
              ${DARK_SELECTOR} {
                ${LIGHT_SELECTOR} {
                  --color-${modeAwareColorName}: ${Color(darkColor).rgb().array().join(" ")};
                }
              }
            `
          }
