 ...
 */
export const tailwindThemeFromColor = (
	colorsMap: any,
	scheme: any,
	contrast: any
) => {
	...
	Object.entries(AllMaterialDynamicColors).forEach(
		([name, DynamicColor]: [string, any]) => {
	...
	Object.keys(extraColors).forEach((colorName) => {
		...
		colors[`${kebabName}-container-light`] = hexFromArgb(
			light.colorContainer
		)
